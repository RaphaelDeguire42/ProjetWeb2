...
export class UneBouteilleComponent {
  ...
  openAjouterBouteilleDialog(id_bouteille:number){
    const dialogRef = this.dialog.open(AjouterBouteilleDialogComponent, {width: '450px', height: '600px', data: { id_bouteille }});
  ...
  openModifierBouteilleDialog(id_bouteille:number){
    const dialogRef = this.dialog.open(ModifierBouteilleDialogComponent, {width: '350px', data: { id_bouteille, ...this.bouteille }});
  ...
  supprimerBouteille(id_bouteille_cellier: number): void {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {width: '350px', data: 'Êtes-vous certain de vouloir supprimer cette bouteille?'});
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  // Creer le boutton pour partager une bouteille de la saq sur facebook
  ...
  // Connecte le boutton facebook à la clé développeur
